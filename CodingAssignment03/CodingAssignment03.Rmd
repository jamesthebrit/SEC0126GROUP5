...
```{r q1.1}
insurance.new <- insurance[-sample(1:nrow(insurance), 3), ]
```


```{r q1.2}
summary(insurance.new)
```

```{r q1.3}
sapply(insurance.new[,1:4], sd)
...
````{r 3i}
See Question 1 From Coding Assignment 2

Model Summary

Review Gauss-Markov Asssumptions And Solutions
````
